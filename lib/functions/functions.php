...
function addBuilding($address,$postal_code,$description,$image)
{
	$address = secureInput($address);
	$postal_code = secureInput($postal_code);
	$description = secureInput($description);
	$image = secureInput($image);
	$sql = "INSERT INTO buildings (address,postal_code,description,image) VALUES ('".$address."','".$postal_code."','".$description."','".$image."')";
	$res = mysql_query($sql) or die(mysql_error());
	if(!$res) return 4;
		return 99;
...
//----------Function for editing building by admin----------
function editBuilding($id,$address,$postal_code,$description)
{
	$id = secureInput($id);
	$address = secureInput($address);
	$postal_code = secureInput($postal_code);
	...
	$sql = "UPDATE buildings SET address = '" . $address . "', postal_code = '" . $postal_code . "', description = '" . $description . "' WHERE id = '" . $id . "'";		
			$res = mysql_query($sql) or die(mysql_error());
				if(!$res) return 4;
				return 99;
...
//----------Function for getting building records----------
function getBuildingRecords($id)
{
	global $getbuilding;
	...
	$c=0;
	while ($a_row = mysql_fetch_array($res)) {
		$getbuilding[$c]["id"] = $a_row["id"];
		$getbuilding[$c]["address"] = $a_row["address"];
		$getbuilding[$c]["postal_code"] = $a_row["postal_code"];
		$getbuilding[$c]["description"] = $a_row["description"];
		$getbuilding[$c]["image"] = $a_row["image"];
		$getbuilding[$c]["active"] = $a_row["active"];
...
function getSpaceRecords($id)
{
	...
	$c=0;
	while ($a_row = mysql_fetch_array($res)) {
		$getspace[$c]["space_id"] = $a_row["space_id"];
		$getspace[$c]["building_id"] = $a_row["building_id"];
		$getspace[$c]["suite_number"] = $a_row["suite_number"];
		$getspace[$c]["square_footage"] = $a_row["square_footage"];
		$getspace[$c]["lease_term"] = $a_row["lease_term"];
		$getspace[$c]["description"] = $a_row["description"];
		$getspace[$c]["first_name"] = $a_row["first_name"];
		$getspace[$c]["last_name"] = $a_row["last_name"];
		$getspace[$c]["email"] = $a_row["email"];
		$getspace[$c]["phone_number"] = $a_row["phone_number"];
		$getspace[$c]["monthly_rents"] = $a_row["monthly_rents"];
		$getspace[$c]["space_active"] = $a_row["space_active"];
...
//----------Function for finding building address----------
function findBuildingRecords($address)
{
	global $findbuilding; 
	...
	while ($row = mysql_fetch_array($res)) {
		$findbuilding[$c]["id"] = $row["id"];
		$findbuilding[$c]["address"] = $row["address"];
		$findbuilding[$c]["postal_code"] = $row["postal_code"];
		$findbuilding[$c]["description"] = $row["description"];
...
//----------Function for deleting building by admin----------
function deleteBuilding($id)
{
	$sql = "SELECT * FROM buildings WHERE id = '".$id."'";
	$res = mysql_query($sql);
	if ($res){
		$del = "DELETE FROM buildings WHERE id = '".$id."'"; 
		$result = mysql_query($del);
			if($result)
				return 99;
					return 1;
...
//----------Function for suspending building by admin----------
function suspendBuilding($id)
{
...
//----------Function for reactivating building by admin----------
function unsuspendBuilding($id)
{
	$sql = "SELECT id,active FROM buildings WHERE id = '".$id."'"; 
	$res = mysql_query($sql);
	if ($res){
		$update = "UPDATE buildings SET active = 1 WHERE id = '".$id."'";
		$result = mysql_query($update);
			if ($result)
				return 99;
					return 1;
...
//----------Function for deleting space by admin----------
function deleteSpace($id)
{
	$sql = "SELECT * FROM spaces WHERE space_id = '".$id."'";
	$res = mysql_query($sql);
	if ($res){
		$del = "DELETE FROM spaces WHERE space_id = '".$id."'"; 
		$result = mysql_query($del);
			if($result)
				return 99;
					return 1;
...
{
	$sql = "SELECT space_id,space_active FROM spaces WHERE space_id = '".$id."'"; 
	$res = mysql_query($sql);
	if ($res){
		$update = "UPDATE spaces SET space_active = 0 WHERE space_id = '".$id."'";
		$result = mysql_query($update);
			if ($result)
				return 99;
					return 1;
...
//----------Function for reactivating space by admin----------
function unsuspendSpace($id)
{
	$sql = "SELECT space_id,space_active FROM spaces WHERE space_id = '".$id."'"; 
	$res = mysql_query($sql);
	if ($res){
		$update = "UPDATE spaces SET space_active = 1 WHERE space_id = '".$id."'";
		$result = mysql_query($update);
			if ($result)
				return 99;
					return 1;
